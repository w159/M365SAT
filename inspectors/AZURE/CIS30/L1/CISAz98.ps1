...
function Build-CISAz98
{
    ...
    $inspectorobject = New-Object PSObject -Property @{
        UUID             = "CISAz98"
        ID               = "9.8"
        Title            = "(L1) Ensure that 'Python version' is currently supported (if in use)"
        ...
        Remediation      = 'Set-AzWebApp -AssignIdentity $True -ResourceGroupName <resource_Group_Name> -Name <App_Name>'
        References       = @(
...
function Audit-CISAz98
{
	try
	{
        ...
        {
            $FinalObject = Build-CISAz98 -ReturnedValue $Violation -Status "FAIL" -RiskScore "0" -RiskRating "Informational"
            return $FinalObject
        }

        $FinalObject = Build-CISAz98 -ReturnedValue "No violations found" -Status "PASS" -RiskScore "0" -RiskRating "None"
        return $FinalObject
    }
    catch
    {
        $EndObject = Build-CISAz98 -ReturnedValue "UNKNOWN" -Status "UNKNOWN" -RiskScore "0" -RiskRating "UNKNOWN"
