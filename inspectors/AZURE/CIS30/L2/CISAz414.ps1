...
function Build-CISAz414
{
    ...
    $inspectorobject = New-Object PSObject -Property @{
        UUID             = "CISAz414"
        ID               = "4.14"
        Title            = "(L2) Ensure Storage Logging is Enabled for Table Service for 'Read', 'Write', and 'Delete' Requests"
        ...
        Remediation      = "You can change the settings by executing the following PowerShell command: Set-AzStorageServiceLoggingProperty -ServiceType Table -LoggingOperations read,write,delete -RetentionDays 90 -Context $MyContextObject"
        References       = @(
...
function Audit-CISAz414
{
    try
    {
        ...
        foreach ($Context in $Contexts) {
            try {
                ...
                $Logging = Get-AzStorageServiceLoggingProperty -ServiceType Table -Context $Cont -ErrorAction SilentlyContinue
        ...
        if ($Violation.Count -gt 0) {
            $FinalObject = Build-CISAz414 -ReturnedValue $Violation -Status "FAIL" -RiskScore "2" -RiskRating "Low"
            return $FinalObject
        }
        else {
            $FinalObject = Build-CISAz414 -ReturnedValue "No violations found" -Status "PASS" -RiskScore "0" -RiskRating "None"
            return $FinalObject
        }
    ...
    {
        $EndObject = Build-CISAz414 -ReturnedValue "UNKNOWN" -Status "UNKNOWN" -RiskScore "0" -RiskRating "UNKNOWN"
