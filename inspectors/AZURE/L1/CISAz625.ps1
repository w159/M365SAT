...
function Build-CISAz625
{
    ...
    $inspectorobject = New-Object PSObject -Property @{
        UUID             = "CISAz625"
        ID               = "6.2.5"
        Title            = "(L1) Ensure that Activity Log Alert exists for Create or Update Security Solution"
        ...
        Impact           = "Failure to monitor security solution changes may delay the detection of suspicious activity."
        Remediation      = "Use the following PowerShell command to create the alert: New-AzActivityLogAlert"
        References       = @(
...
function Audit-CISAz625
{
    try
    {
        ...
        if ($Violation.Count -gt 0) {
            $FinalObject = Build-CISAz625 -ReturnedValue $Violation -Status "FAIL" -RiskScore "2" -RiskRating "Low"
            return $FinalObject
        }
        else {
            $FinalObject = Build-CISAz625 -ReturnedValue "No violations found" -Status "PASS" -RiskScore "0" -RiskRating "None"
            return $FinalObject
        }
    ...
    {
        $EndObject = Build-CISAz625 -ReturnedValue "UNKNOWN" -Status "UNKNOWN" -RiskScore "0" -RiskRating "UNKNOWN"
        Write-WarningLog 'The Inspector: {inspector} was terminated!' -PropertyValues $_.InvocationInfo.ScriptName
        Write-ErrorLog 'An error occurred on line {line} char {char} : {error}' -ErrorRecord $_ -PropertyValues $_.InvocationInfo.ScriptLineNumber, $_.InvocationInfo.OffsetInLine, $_.InvocationInfo.Line
        return $EndObject
    }
}
return Audit-CISAz625
