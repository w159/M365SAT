...
function Build-CISAz623
{
    ...
    $inspectorobject = New-Object PSObject -Property @{
        UUID             = "CISAz623"
        ID               = "6.2.3"
        Title            = "(L1) Ensure that Activity Log Alert exists for Create or Update Network Security Group"
        ...
        Impact           = "Failure to monitor these events can increase the time to detect unauthorized changes to network security groups."
...
function Audit-CISAz623
{
    try
    {
        ...
        if ($Violation.Count -gt 0) {
            $FinalObject = Build-CISAz623 -ReturnedValue $Violation -Status "FAIL" -RiskScore "2" -RiskRating "Low"
            return $FinalObject
        }
        else {
            $FinalObject = Build-CISAz623 -ReturnedValue "No violations found" -Status "PASS" -RiskScore "0" -RiskRating "None"
            return $FinalObject
        }
    ...
    {
        $EndObject = Build-CISAz623 -ReturnedValue "UNKNOWN" -Status "UNKNOWN" -RiskScore "0" -RiskRating "UNKNOWN"
