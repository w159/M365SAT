...
function Build-CISAz621
{
    ...
    $inspectorobject = New-Object PSObject -Property @{
        UUID             = "CISAz621"
        ID               = "6.2.1"
        Title            = "(L1) Ensure that Activity Log Alert exists for Create Policy Assignment"
        ...
        Impact           = "Failure to detect such changes can increase the time to respond to potentially malicious modifications."
...
function Audit-CISAz621
{
    try
    {
        ...
        if ($Violation.Count -gt 0) {
            $FinalObject = Build-CISAz621 -ReturnedValue $Violation -Status "FAIL" -RiskScore "2" -RiskRating "Low"
            return $FinalObject
        }
        else {
            $FinalObject = Build-CISAz621 -ReturnedValue "No violations found" -Status "PASS" -RiskScore "0" -RiskRating "None"
            return $FinalObject
        }
    ...
    {
        $EndObject = Build-CISAz621 -ReturnedValue "UNKNOWN" -Status "UNKNOWN" -RiskScore "0" -RiskRating "UNKNOWN"
