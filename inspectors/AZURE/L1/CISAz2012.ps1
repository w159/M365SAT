...
function Build-CISAz2012
{
    ...
    $inspectorobject = New-Object PSObject -Property @{
        UUID             = "CISAz2012"
        ID               = "2.12"
        Title            = "(L1) Ensure 'User consent for applications' is set to 'Do not allow user consent'"
...
function Audit-CISAz2012
{
	try
	{
		...
		{
			$finalobject = Build-CISAz2012 -ReturnedValue ($userConsent.defaultUserRolePermissions.permissionGrantPoliciesAssigned) -Status "FAIL" -RiskScore "12" -RiskRating "Medium"
			return $endobject
		}
		else
		{
			$endobject = Build-CISAz2012 -ReturnedValue ($userConsent.defaultUserRolePermissions.permissionGrantPoliciesAssigned) -Status "PASS" -RiskScore "0" -RiskRating "None"
			Return $endobject
		}
		return $null
	}
	...
	{
		$endobject = Build-CISAz2012 -ReturnedValue "UNKNOWN" -Status "UNKNOWN" -RiskScore "0" -RiskRating "UNKNOWN"
