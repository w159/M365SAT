...
function Build-CISAz334
{
    ...
    $inspectorobject = New-Object PSObject -Property @{
        UUID             = "CISAz334"
        ID               = "3.3.4"
        Title            = "(L1) Ensure that the Expiration Date is set for all Secrets in Non-RBAC Key Vaults"
        ...
        Remediation      = 'Use the PowerShell Script to remediate the issue: Set-AzKeyVaultSecret -VaultName <Vault Name> -Name <Secret Name> -Expires <DateTime>'
        References       = @(
...
function Audit-CISAz334
{
    try
    {
        ...
        {
            $endobject = Build-CISAz334 -ReturnedValue ($Violation.Count) -Status "FAIL" -RiskScore "0" -RiskRating "Informational"
            return $endobject
        }
        else
        {
            $endobject = Build-CISAz334 -ReturnedValue "No violations found" -Status "PASS" -RiskScore "0" -RiskRating "None"
            Return $endobject
        }
        return $null
    }
    ...
    {
        $endobject = Build-CISAz334 -ReturnedValue "UNKNOWN" -Status "UNKNOWN" -RiskScore "0" -RiskRating "UNKNOWN"
