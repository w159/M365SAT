...
function Audit-CISAz26
{
	try
	{
		...
		{
			$endobject = Build-CISAz26 -ReturnedValue ($MethodsRequired.lockoutThreshold) -Status "FAIL" -RiskScore "3" -RiskRating "Low"
			return $endobject
		}
		else
		{
			$endobject = Build-CISAz26 -ReturnedValue ($MethodsRequired.lockoutThreshold) -Status "PASS" -RiskScore "0" -RiskRating "None"
			Return $endobject
		}
		return $null
	}
	...
	{
		$endobject = Build-CISAz26 -ReturnedValue "UNKNOWN" -Status "UNKNOWN" -RiskScore "0" -RiskRating "UNKNOWN"
