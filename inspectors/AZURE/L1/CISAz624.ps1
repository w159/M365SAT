...
function Build-CISAz624
{
    ...
    $inspectorobject = New-Object PSObject -Property @{
        UUID             = "CISAz624"
        ID               = "6.2.4"
        Title            = "(L1) Ensure that Activity Log Alert exists for Delete Network Security Group"
        ProductFamily    = "Microsoft Azure"
        DefaultValue     = "By default, no monitoring alerts are created."
        ExpectedValue    = "An Activity Log Alert Rule for Microsoft.Network/networkSecurityGroups/delete"
        ...
        Impact           = "Failure to monitor delete events for network security groups increases the risk of undetected malicious activity affecting network security."
...
function Audit-CISAz624
{
    try
    {
        ...
        {
            $LogAlert = Get-AzActivityLogAlert -SubscriptionId $Subscription.Id | Where-Object {$_.ConditionAllOf.Equal -match "Microsoft.Network/networkSecurityGroups/delete"} | Select-Object Location, Name, Enabled, ResourceGroupName, ConditionAllOf
        ...
        {
            $FinalObject = Build-CISAz624 -ReturnedValue $Violation -Status "FAIL" -RiskScore "2" -RiskRating "Low"
            return $FinalObject
        }
        else
        {
            $FinalObject = Build-CISAz624 -ReturnedValue "No violations found" -Status "PASS" -RiskScore "0" -RiskRating "None"
            return $FinalObject
        }
